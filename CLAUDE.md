...
Releasing is bump `Cargo.toml`, commit, then publish a `v*` release (`gh release create v1.4.0 --generate-notes`, or the GitHub UI). `docker.yml` fires on the `v*` tag push and pushes multi-arch images to `ghcr.io/<owner>/quarkdrive-webdav`, tagging `:latest` only for tag builds. `release.yml` fires on `release: published` and attaches six target triples to that release — it must never set `body` or `generate_release_notes`, because the upload step runs once per matrix target and those options append to the release body every time (v1.4.0 shipped with the changelog repeated seven times that way). Both workflows refuse to publish if the tag disagrees with the `Cargo.toml` version or if `cargo test` fails; that gate is in CI rather than a local script because a release can be published from the GitHub UI, where nothing local runs.
